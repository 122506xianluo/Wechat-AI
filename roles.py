...
class Roles:
    ...
    def list(self):
        with self.storage._connection() as c:
            return [
                dict(r)
                for r in c.execute("SELECT * FROM roles ORDER BY is_default DESC,id")
            ]

    def resolve(self, chat_id, principal_id=None):
        with self.storage._connection() as c:
            row = c.execute(
                """SELECT r.* FROM role_bindings b JOIN roles r ON r.id=b.role_id
            ...
            if not row:
                row = c.execute(
                    "SELECT * FROM roles WHERE is_default=1 AND enabled=1"
                ).fetchone()
            if not row:
    ...
    def save(self, data, actor, role_id=None):
        ...
        with self.storage.transaction() as c:
            ...
            old = (
                c.execute("SELECT * FROM roles WHERE id=?", (role_id,)).fetchone()
                if role_id
                else None
            )
            ...
            if old:
                c.execute(
                    "INSERT INTO role_revisions(role_id,revision,payload) VALUES(?,?,?)",
                    (
                        role_id,
                        old["revision"],
    ...
    def revisions(self, role_id):
        with self.storage._connection() as c:
            return [
                dict(r)
                for r in c.execute(
                    "SELECT * FROM role_revisions WHERE role_id=? ORDER BY revision DESC",
                    (role_id,),
                )
            ]
    ...
    def rollback(self, role_id, revision, actor):
        with self.storage._connection() as c:
            row = c.execute(
                "SELECT payload FROM role_revisions WHERE role_id=? AND revision=?",
                (role_id, revision),
            ).fetchone()
        if not row:
        ...
        return self.save(
            {k: v for k, v in data.items() if k in self.FIELDS}, actor, role_id
        )
    ...
    def delete(self, role_id, actor, replacement=None):
        with self.storage.transaction() as c:
            ...
            if c.execute(
                "SELECT 1 FROM role_bindings WHERE role_id=?", (role_id,)
            ).fetchone():
                if (
                    not replacement
                    or replacement == role_id
                    or not c.execute(
                        "SELECT 1 FROM roles WHERE id=? AND enabled=1", (replacement,)
                    ).fetchone()
                ):
                    raise ValueError("使用中的角色必须指定有效替代角色")
                c.execute(
                    "UPDATE role_bindings SET role_id=? WHERE role_id=?",
                    (replacement, role_id),
                )
    ...
    def bind(self, role_id, chat_id, principal_id, actor, *, self_select=False):
        with self.storage.transaction() as c:
            role = c.execute(
                "SELECT * FROM roles WHERE id=? AND enabled=1", (role_id,)
            ).fetchone()
            if not role:
            ...
            if self_select:
                ...
                require_manager(c, actor)
            c.execute(
                "DELETE FROM role_bindings WHERE chat_id IS ? AND principal_id IS ?",
                (chat_id, principal_id),
            )
            c.execute(
                "INSERT INTO role_bindings(role_id,chat_id,principal_id) VALUES(?,?,?)",
                (role_id, chat_id, principal_id),
            )
            record_audit(
                c,
                "role.bind",
