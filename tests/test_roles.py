...
def test_roles_precedence_and_revision(tmp_path):
    s = Storage(tmp_path)
    s.register_targets(["Synthetic"], [])
    r = Roles(s)
    chat = s.chat_id("private", "Synthetic")
    original = r.resolve(chat)
    role = r.save(
        {"name": "Synthetic role", "system_prompt": "Synthetic prompt"}, LOCAL_OWNER
    )
    ...
    with pytest.raises(ValueError):
        r.delete(original["id"], LOCAL_OWNER)
    with pytest.raises(ValueError):
        r.save({"name": "Invalid", "temperature": 20}, LOCAL_OWNER)
